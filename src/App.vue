...
onMounted(() => {
  ...
  data.textures.video = new VideoTexture(
    "video",
    BASE_URL + "videos/cat.mp4",
    data.scene,
    false,
    false,
  ...
  let triangle_indices = [
    0,
    1,
    2, // connect vertices 0,1,2 to make a triangle
    0,
    2,
    3, // connect vertices 0,2,3 to make a triangle
  ];
  ...
  data.scene.onBeforeRenderObservable.add(() => {
    ...
    if (data.textures[data.selected_texture] !== null) {
      data.materials[data.filter].setTexture(
        "image",
        data.textures[data.selected_texture]
      );
    }
    ...
    // update uniform
    rect.material.setFloat("time", time);
    rect.material.setFloat("slider", data.sliderValue);
  });
...
<template>
  ...
  <div id="gui">
    <div
      v-for="f in data.filter_options"
      :id="f.id + '_button'"
      class="filter_button"
      :selected="f.selected"
      @click="selectFilter"
